...
void  PRL_Init (UINT8 u8PortNum)
{
    ...
    HOOK_DEBUG_PORT_STR (u8PortNum,"PRL: Intialisation Done\r\n");
}
					   ...
					   PRLTxCallback pfnTxCallback, UINT32  u32PkdPEstOnTxStatus)
{
    ...
    /* Hook for PRL Tx*/
    CONFIG_HOOK_PRL_TX_MSG_PRE_PROCESS(u8PortNum, u8SOPType, u32Header, pu8DataBuffer, pfnTxCallback, u32PkdPEstOnTxStatus);
	...
	if ((!gasChunkSM [u8PortNum].u8EnableChunkSM) && (PRL_IS_EXTENDED_MSG(u32Header)))
	{
		...
		*/
		...
		HOOK_DEBUG_PORT_STR (u8PortNum, "PRL: Chunk SM Enabled\r\n");
	...
	switch (u8SOPType)
	{
		...
		{
			u8MsgId = 0;
			CONFIG_HOOK_DEBUG_STRING ("Wrong SOP* type detected\r\n");
			break;
		}
	}
    ...
    /* Hook for PRL TX*/
    CONFIG_HOOK_PRL_TX_MSG_POST_PROCESS(u8PortNum, u8SOPType, u32Header, pu8DataBuffer, pfnTxCallback, u32PkdPEstOnTxStatus);
	...
	{	  
        ...
        HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_TX_MSG_BUFFERED_ON_CA: Tx Msg Buffered due to CA\r\n");
            ...
            (PRL_SOP_TYPE == gasPRLRecvBuff[u8PortNum].u8SOPtype))     /* Checks whether received message is PRL_SOP_TYPE type*/
	{
        ...
        HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_TX_MSG_DISCARD_ON_RCV: Tx Msg Discarded on Recv\r\n");
	...
	HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_TX_MSG_ON_LINE: Tx Msg sent on line\r\n");
...
void PRL_SendCableorHardReset (UINT8 u8PortNum, UINT8 u8CableorHardReset, PRLTxCallback pfnTxCallback, UINT32  u32PkdPEstOnTxStatus)
{
	...
	HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_TX_HARD_RESET_SENT: Hard Reset sent on line\r\n");
...
void PRL_EnableRx (UINT8 u8PortNum, bool bEnable)
{
	if (bEnable)
	{
        ...
        HOOK_DEBUG_PORT_STR (u8PortNum,"PRL: Receiver enabled\r\n");
	}
	else
	{
...
UINT8 PRL_ReceiveMsg (UINT8 u8PortNum, UINT8  *pu8SOPType, UINT32 *pu32Header, UINT8 *pu8DataBuffer, PRLRxCallback pfnRxCallback)
{
    ...
    /* Hook function for Receiver*/
	CONFIG_HOOK_PRL_RX_MSG_PRE_PROCESS(u8PortNum);
    ...
    if (u8Return & PRL_RET_MSG_RCVD)
    {
        ...
        *pu32Header =  gasPRLRecvBuff[u8PortNum].u16Header;
        ...
        HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_RX_PKT_PASSED_TO_PE: Rx Msg received passed to PE\r\n");
    ...
    else if (u8Return & PRL_RET_EXT_MSG_RCVD)
	{
		...
		*pu32Header = PRL_FORM_COMBINED_MSG_HEADER (gasExtendedMsgBuff [u8PortNum].u16ExtendedMsgHeader, 
		...
		HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_EXTN_RX_PKT_PASSED_TO_PE: Extended Msg received passed to PE\r\n");
	...
	else if(gasPRL [u8PortNum].u8RxError)
    {
		...
		HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_RX_CHUNK_RCV_ERROR: Rx Chunk Error \r\n");
	}
    #endif
    else
    {
    ...
    CONFIG_HOOK_PRL_RX_MSG_POST_PROCESS(u8PortNum);
...
void PRL_HRorCRCompltIndicationFromPE (UINT8 u8PortNum)
{
	...
	HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_HR_COMPLETE: PRL is informed about HR complete");
...
/***********************************************Extended Message Handling**************************************************************/
    ...
    if (PRL_IS_EXTENDED_MSG(gasPRLRecvBuff [u8PortNum].u16Header))
	{
		...
		UINT16 u16ExtendedMsgHeader = MAKE_UINT16(gasPRLRecvBuff [u8PortNum].u8DataObj[BYTE_1],gasPRLRecvBuff [u8PortNum].u8DataObj[BYTE_0]);
		...
		if (PRL_IS_REQUEST_CHUNK_MSG(u16ExtendedMsgHeader))
		{
			...
			{
			  	HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_CHUNK_REQUEST_RCV: Chunk Request received");
			...
			{
			  	HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_TCH_ERROR: Request Chunk number mismatch");
		...
		else
		{
		  	...
		  	if (PRL_FIRST_CHUNK_PACKET == PRL_GET_CHUNK_NUMBER(u16ExtendedMsgHeader))
			{
               /* if Chunk SM Chunk message is already in process & other message is received*/
                ...
                {
                    HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_CHUNK_UNEXPECTED_MSG_RCV: Unexpected Chunk msg received");
					...
					&& PRL_IS_MSG_CHUNKED(u16ExtendedMsgHeader))
			{
				...
				{
				  	HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_CHUNK_RESPONSE_RCV: Chunk Response Received");
			...
			{
			  	HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_CHUNK_RECV_ERROR: Response Chunk number mismatch");
    ...
    #endif /* for INCLUDE_PD_3_0 */
	{
	  	...
	  	if ((gasChunkSM [u8PortNum].u8EnableChunkSM) && (PRL_GET_MESSAGE_TYPE(gasPRLRecvBuff [u8PortNum].u16Header) != PE_CTRL_PING))
		{
		  	...
		  	if ((gasChunkSM [u8PortNum].u8ChunkState == PRL_RCH_EXPECT_RESPONSE_CHUNK_WAIT_ST)
                ...
                || (gasChunkSM [u8PortNum].u8ChunkState == PRL_RCH_WAIT_FOR_CHUNK_REQUEST_STATUS_ST))
			{
                HOOK_DEBUG_PORT_STR (u8PortNum,"PRL_CHUNK_UNEXPECTED_MSG_RCV: Unexpected msg received other than Ping & Chunk msg");
...
UINT8 PRL_SetCollisionAvoidance (UINT8 u8PortNum, UINT8 u8Enable)
{
	...
	if (u8Enable)
	{
		...
		gasChunkSM [u8PortNum].u8CAorChunkSMTimerID = PDTimer_Start (CONFIG_PRL_SINKTX_TIMEOUT_MS,\
                                                        PRL_CASinkTxTimerOut_TimerCB, u8PortNum, NULL);
		
		
		/* u8Txstate is set to PRL_Tx_CA_SRC_SINKTXTIMER_ON_ST*/
		PRL_ChangeTxState (u8PortNum, PRL_Tx_CA_SRC_SINKTXTIMER_ON_ST); 
		
        HOOK_DEBUG_PORT_STR (u8PortNum,"PRL: CONFIG_PRL_SINK_TX_TIMEOUT_MS is set\r\n");
	}
	else
	{
...
void PRL_CASinkTxTimerOut_TimerCB (UINT8 u8PortNum, UINT8 u8PESubstate)
{
...
void PRL_CommitPendingTxOnCAISR (UINT8 u8PortNum)
{
    ...
    else if (PRL_TX_MSG_BUFFERED_ON_CA_ST == gasPRL [u8PortNum].u8TxStateISR)
    {
        ...
        HOOK_DEBUG_PORT_STR (u8PortNum,"PRL: Buffered Tx Msg sent on line\r\n");
    }
    else
    {
        /* PRL_TX_IDLE_ST is assigned to PRL state*/
...
void PRL_ChunkStateChange_TimerCB (UINT8 u8PortNum, UINT8 u8ChunkState)
{
	...
	HOOK_DEBUG_PORT_STR (u8PortNum, "PRL: ChunkTimer time out \r\n");
...
{
    /*Hook for Chunk SM*/
    CONFIG_HOOK_PRL_CHUNK_SM(u8PortNum);
	...
	switch (gasChunkSM [u8PortNum].u8ChunkState)
	{
		...
		{
		  	HOOK_DEBUG_PORT_STR (u8PortNum, "PRL: Chunk SM PRL_RCH_SEND_CHUNK_REQUEST_ST\r\n");
		...
		case PRL_TCH_SEND_RESPONSE_CHUNK_ST:
		{
            ...
            else
            {
			    ...
			    HOOK_DEBUG_PORT_STR (u8PortNum, "PRL: Chunk SM PRL_TCH_SEND_RESPONSE_CHUNK_ST\r\n");
		...
		{
		  	HOOK_DEBUG_PORT_STR (u8PortNum, "PRL: Chunk SM PRL_TCH_CHUNK_TX_ERROR_ST\r\n");
