    ...
    UINT8   *pu8RcvMsgDataBuf)  /* */
{
    ...
    if(ePE_FWUP_MANIFEST_SS == PE_FWUP_GET_CURRENT_SUBSTATE(u8PortNum))
    {
        ...
        CONFIG_HOOK_BOOT_UPDATABLE_APPLICATION();
    }
    ...
    UINT16  u16ExtendedMsgHeader)   /* */
{
    ...
    case PE_FWUP_EVT_PDFUDATANR_REQ:
        ...
        if (PE_FWUP_TRANSFER_PHASE_COMPLETE != gsPdfuInfo.u8TransferPhaseComplete)
        {
            u8Status = CONFIG_HOOK_PROGRAM_FWBLOCK(pu8DataBuffer,
...
UINT16 PE_FwUpdtGetFWIDRequest(void)
{
    ...
    stGetFWID.u16VID = (UINT16)CONFIG_PRODUCT_VID;

    /*Marked as TBD in Hermes interface*/
    stGetFWID.u16PID = (UINT16)CONFIG_PRODUCT_PID;
    ...
    stGetFWID.u16FWVersion4 = (((UINT16) (((UINT16)PD_FW_CURRENT_FW_VERSION) >> 8u)) | ((UINT16)PD_FW_CURRENT_FW_VERSION & (UINT16)0xFFu));

    /*Current image bank, u8CurrentMemory was stored when booting*/
    stGetFWID.u8ImageBank = CONFIG_HOOK_GETCURRENT_IMAGEBANK();
    ...
    (void)CONFIG_HOOK_MEMCPY(&gsPdfuInfo.pu8ResponseBuffer[2], (UINT8 *) &stGetFWID, u16Length);
...
UINT8 PE_FwUpdtPDFUInitateRequest(UINT8   u8PortNum, UINT8   u8WaitTime) 
{
    ...
    if (CONFIG_UPDATABLE_IMAGEBANK_INDEX != CONFIG_HOOK_GETCURRENT_IMAGEBANK())
    {
    ...
    else
    {
        ...
        CONFIG_HOOK_BOOT_FIXEDAPPLICATION();
    }
...
UINT8 PE_FwUpdtPDFUValidateRequest(UINT8   u8PortNum)  
{
   ...
   // gsPdfuInfo.u16PDFUResponseLength = PE_FWUP_RESPBUFF_STATUS_INDEX;
    u8ValidationStatus = (UINT8)CONFIG_HOOK_VALIDATE_FIRMWARE();
    ...
    UINT16  u16ExtendedMsgHeader)   /* */
{
    ...
    if (PE_FWUP_TRANSFER_PHASE_COMPLETE != gsPdfuInfo.u8TransferPhaseComplete)
    {
        ...
        {
            /** To process the request \a CONFIG_HOOK_PROGRAM_FWBLOCK \norm is invoked.
            The response message contents are copied into \a gsPdfuInfo.pu8ResponseBuffer*/
            u8Status = CONFIG_HOOK_PROGRAM_FWBLOCK(pu8DataBuffer, u16DataBlockSize);
        }
        UINT16 u16PDFUResponseLength = (UINT16)PE_FWUP_RESPBUFF_STATUS_INDEX;
