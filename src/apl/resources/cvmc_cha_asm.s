...
;------------------------------------------------------------------------------
	...
	.global _cvmc_cha_Update
_cvmc_cha_Update:    ; provide global scope to routine
...
; Check for upper limit violation
	...
	bra CVMC_CHA_CLAMP_MAX_OVERRIDE    ; jump to override label if control output > upper limit
	bclr w12, #NPMZ16_STATUS_USAT    ; clear upper limit saturation flag bit
	bra CVMC_CHA_CLAMP_MAX_EXIT    ; jump to exit
	CVMC_CHA_CLAMP_MAX_OVERRIDE:
	mov w6, w4    ; override controller output
	bset w12, #NPMZ16_STATUS_USAT    ; set upper limit saturation flag bit
	CVMC_CHA_CLAMP_MAX_EXIT:
...
; Check for lower limit violation
	...
	bra CVMC_CHA_CLAMP_MIN_OVERRIDE    ; jump to override label if control output < lower limit
	bclr w12, #NPMZ16_STATUS_LSAT    ; clear lower limit saturation flag bit
	bra CVMC_CHA_CLAMP_MIN_EXIT    ; jump to exit
	CVMC_CHA_CLAMP_MIN_OVERRIDE:
	mov w6, w4    ; override controller output
	bset w12, #NPMZ16_STATUS_LSAT    ; set lower limit saturation flag bit
	CVMC_CHA_CLAMP_MIN_EXIT:
