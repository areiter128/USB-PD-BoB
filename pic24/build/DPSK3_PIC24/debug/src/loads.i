# 1 "src/loads.c"
# 1 "C:\\_dsmps\\DPSK3\\Troubleshooting\\__src__\\dpsk3-firmware\\pic24//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "src/loads.c"
# 23 "src/loads.c"
...
# 75 "mcc_generated_files/interrupt_manager.h"
void INTERRUPT_Initialize(void);
# 104 "mcc_generated_files/interrupt_manager.h"
inline static void INTERRUPT_GlobalEnable(void)
{
    __builtin_disi(0x0000);
}
# 135 "mcc_generated_files/interrupt_manager.h"
...
# 52 "mcc_generated_files/pin_manager.h"
# 1 "c:\\program files (x86)\\microchip\\xc16\\v1.36\\bin\\bin\\../..\\support\\generic\\h/xc.h" 1 3 4
# 646 "c:\\program files (x86)\\microchip\\xc16\\v1.36\\bin\\bin\\../..\\support\\generic\\h/xc.h" 3 4
# 1 "c:\\program files (x86)\\microchip\\xc16\\v1.36\\bin\\bin\\../..\\support\\PIC24F\\h/p24FJ64GA004.h" 1 3 4
# 41 "c:\\program files (x86)\\microchip\\xc16\\v1.36\\bin\\bin\\../..\\support\\PIC24F\\h/p24FJ64GA004.h" 3 4
...
# 42 "c:\\program files (x86)\\microchip\\xc16\\v1.36\\bin\\bin\\../..\\support\\PIC24F\\h/p24FJ64GA004.h" 2 3 4
# 647 "c:\\program files (x86)\\microchip\\xc16\\v1.36\\bin\\bin\\../..\\support\\generic\\h/xc.h" 2 3 4
# 53 "mcc_generated_files/pin_manager.h" 2
# 1403 "mcc_generated_files/pin_manager.h"
void PIN_MANAGER_Initialize(void);
# 29 "src/loads.c" 2
# 1 "../common/h/proto_data.h" 1
# 34 "../common/h/proto_data.h"
typedef union {
    struct {
...
void LoadsInit(void)
{
    ...
    pdata = &data_buck;
    do{pdata->set_still = stills[state_S0]; pdata->set_blink = blinks[state_S0]; pdata->state = fstates[state_S0]; }while(0);
    LATCbits.LATC0 = 0;
    LATCbits.LATC1 = 0;
    LATCbits.LATC2 = 0;
...
static void _LoadSignal(void)
{
    if(data_buck.set_blink & (1<<0))
        LedBlink(0);
    else if(data_buck.set_still & (1<<0))
        LedOn(0);
    else
        LedOff(0);

    if(data_buck.set_blink & (1<<1))
        LedBlink(3);
    else if(data_buck.set_still & (1<<1))
        LedOn(3);
    else
        LedOff(3);

    if(data_buck.set_blink & (1<<2))
        LedBlink(6);
    else if(data_buck.set_still & (1<<2))
        LedOn(6);
    else
        LedOff(6);

    if(data_boost.set_blink & (1<<0))
        LedBlink(1);
    else if(data_boost.set_still & (1<<0))
        LedOn(1);
    else
        LedOff(1);

    if(data_boost.set_blink & (1<<1))
        LedBlink(4);
    else if(data_boost.set_still & (1<<1))
        LedOn(4);
    else
        LedOff(4);

    if(data_boost.set_blink & (1<<2))
        LedBlink(7);
    else if(data_boost.set_still & (1<<2))
        LedOn(7);
    else
        LedOff(7);
}
...
static void _fstate_S0(loads_event_t event, void *ptr)
{
    ...
    {
        case LOADS_SBUTTON_BUCK:
        case LOADS_SBUTTON_BOOST: do{pdata->set_still = stills[state_S1]; pdata->set_blink = blinks[state_S1]; pdata->state = fstates[state_S1]; }while(0); break;
        case LOADS_LBUTTON_BUCK:
        case LOADS_LBUTTON_BOOST: do{pdata->set_still = stills[state_B1]; pdata->set_blink = blinks[state_B1]; pdata->state = fstates[state_B1]; }while(0); break;
        case LOADS_THERM_ALERT: do{pdata->set_still = stills[state_S0t]; pdata->set_blink = blinks[state_S0t]; pdata->state = fstates[state_S0t]; }while(0); break;
        default: break;
    }
}
...
static void _fstate_S1(loads_event_t event, void *ptr)
{
    ...
    {
        case LOADS_SBUTTON_BUCK:
        case LOADS_SBUTTON_BOOST: do{pdata->set_still = stills[state_S2]; pdata->set_blink = blinks[state_S2]; pdata->state = fstates[state_S2]; }while(0); break;
        case LOADS_LBUTTON_BUCK:
        case LOADS_LBUTTON_BOOST: do{pdata->set_still = stills[state_B1]; pdata->set_blink = blinks[state_B1]; pdata->state = fstates[state_B1]; }while(0); break;
        case LOADS_THERM_ALERT: do{pdata->set_still = stills[state_S1t]; pdata->set_blink = blinks[state_S1t]; pdata->state = fstates[state_S1t]; }while(0); break;
        default: break;
    }
}
...
static void _fstate_S2(loads_event_t event, void *ptr)
{
    ...
    {
        case LOADS_SBUTTON_BUCK:
        case LOADS_SBUTTON_BOOST: do{pdata->set_still = stills[state_S3]; pdata->set_blink = blinks[state_S3]; pdata->state = fstates[state_S3]; }while(0); break;
        case LOADS_LBUTTON_BUCK:
        case LOADS_LBUTTON_BOOST: do{pdata->set_still = stills[state_B2]; pdata->set_blink = blinks[state_B2]; pdata->state = fstates[state_B2]; }while(0); break;
        case LOADS_THERM_ALERT: do{pdata->set_still = stills[state_S1t]; pdata->set_blink = blinks[state_S1t]; pdata->state = fstates[state_S1t]; }while(0); break;
        default: break;
    }
}
...
static void _fstate_S3(loads_event_t event, void *ptr)
{
    ...
    {
        case LOADS_SBUTTON_BUCK:
        case LOADS_SBUTTON_BOOST: do{pdata->set_still = stills[state_S0]; pdata->set_blink = blinks[state_S0]; pdata->state = fstates[state_S0]; }while(0); break;
        case LOADS_LBUTTON_BUCK:
        case LOADS_LBUTTON_BOOST: do{pdata->set_still = stills[state_B3]; pdata->set_blink = blinks[state_B3]; pdata->state = fstates[state_B3]; }while(0); break;
        case LOADS_THERM_ALERT: do{pdata->set_still = stills[state_S1t]; pdata->set_blink = blinks[state_S1t]; pdata->state = fstates[state_S1t]; }while(0); break;
        default: break;
    }
}
...
static void _fstate_B1(loads_event_t event, void *ptr)
{
    ...
    {
        case LOADS_SBUTTON_BUCK:
        case LOADS_SBUTTON_BOOST: do{pdata->set_still = stills[state_B2]; pdata->set_blink = blinks[state_B2]; pdata->state = fstates[state_B2]; }while(0); break;
        case LOADS_LBUTTON_BUCK:
        case LOADS_LBUTTON_BOOST: do{pdata->set_still = stills[state_S1]; pdata->set_blink = blinks[state_S1]; pdata->state = fstates[state_S1]; }while(0); break;
        case LOADS_THERM_ALERT: do{pdata->set_still = stills[state_B1t]; pdata->set_blink = blinks[state_B1t]; pdata->state = fstates[state_B1t]; }while(0); break;
        default: break;
    }
}
...
static void _fstate_B2(loads_event_t event, void *ptr)
{
    ...
    {
        case LOADS_SBUTTON_BUCK:
        case LOADS_SBUTTON_BOOST: do{pdata->set_still = stills[state_B3]; pdata->set_blink = blinks[state_B3]; pdata->state = fstates[state_B3]; }while(0); break;
        case LOADS_LBUTTON_BUCK:
        case LOADS_LBUTTON_BOOST: do{pdata->set_still = stills[state_S2]; pdata->set_blink = blinks[state_S2]; pdata->state = fstates[state_S2]; }while(0); break;
        case LOADS_THERM_ALERT: do{pdata->set_still = stills[state_B1t]; pdata->set_blink = blinks[state_B1t]; pdata->state = fstates[state_B1t]; }while(0); break;
        default: break;
    }
}
...
static void _fstate_B3(loads_event_t event, void *ptr)
{
    ...
    {
        case LOADS_SBUTTON_BUCK:
        case LOADS_SBUTTON_BOOST: do{pdata->set_still = stills[state_B1]; pdata->set_blink = blinks[state_B1]; pdata->state = fstates[state_B1]; }while(0); break;
        case LOADS_LBUTTON_BUCK:
        case LOADS_LBUTTON_BOOST: do{pdata->set_still = stills[state_S3]; pdata->set_blink = blinks[state_S3]; pdata->state = fstates[state_S3]; }while(0); break;
        case LOADS_THERM_ALERT: do{pdata->set_still = stills[state_B1t]; pdata->set_blink = blinks[state_B1t]; pdata->state = fstates[state_B1t]; }while(0); break;
        default: break;
    }
}
...
static void _fstate_S0t(loads_event_t event, void *ptr)
{
    ...
    {
        case LOADS_SBUTTON_BUCK:
        case LOADS_SBUTTON_BOOST: do{pdata->set_still = stills[state_S1t]; pdata->set_blink = blinks[state_S1t]; pdata->state = fstates[state_S1t]; }while(0); break;
        case LOADS_LBUTTON_BUCK:
        case LOADS_LBUTTON_BOOST: do{pdata->set_still = stills[state_B1t]; pdata->set_blink = blinks[state_B1t]; pdata->state = fstates[state_B1t]; }while(0); break;
        case LOADS_THERM_SAFE: do{pdata->set_still = stills[state_S0]; pdata->set_blink = blinks[state_S0]; pdata->state = fstates[state_S0]; }while(0); break;
        default: break;
    }
}
...
static void _fstate_S1t(loads_event_t event, void *ptr)
{
    ...
    {
        case LOADS_SBUTTON_BUCK:
        case LOADS_SBUTTON_BOOST: do{pdata->set_still = stills[state_S0t]; pdata->set_blink = blinks[state_S0t]; pdata->state = fstates[state_S0t]; }while(0); break;
        case LOADS_LBUTTON_BUCK:
        case LOADS_LBUTTON_BOOST: do{pdata->set_still = stills[state_B1t]; pdata->set_blink = blinks[state_B1t]; pdata->state = fstates[state_B1t]; }while(0); break;
        case LOADS_THERM_SAFE: do{pdata->set_still = stills[state_S1]; pdata->set_blink = blinks[state_S1]; pdata->state = fstates[state_S1]; }while(0); break;
        default: break;
    }
}
...
static void _fstate_B1t(loads_event_t event, void *ptr)
{
    ...
    {
        case LOADS_LBUTTON_BUCK:
        case LOADS_LBUTTON_BOOST: do{pdata->set_still = stills[state_S1t]; pdata->set_blink = blinks[state_S1t]; pdata->state = fstates[state_S1t]; }while(0); break;
        case LOADS_THERM_SAFE: do{pdata->set_still = stills[state_B1]; pdata->set_blink = blinks[state_B1]; pdata->state = fstates[state_B1]; }while(0); break;
        default: break;
    }
}
