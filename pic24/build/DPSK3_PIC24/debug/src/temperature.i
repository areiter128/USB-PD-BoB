...
therm_event_t TemperatureCheck(uint8_t * pTemperature)
{
    static _Bool stateThermalShutDown = 0;
    static _Bool stateThermalWarning = 0;
    therm_event_t ret = THERM_NO_CHANGE;
    uint8_t temperature = (uint8_t)(((double)(3.3*ADC1_ReadAvg()/1024.0)-0.5) / 0.01);

    if(stateThermalShutDown)
    {
        if(temperature < (75 - 5))
        {
            stateThermalShutDown = 0;
            stateThermalWarning = 1;
            ret = THERM_WARNING;
        }
    }
    else
