...
static const led_row_t led_rows_array[] = {
                                           {(uint16_t * const)&LATB, (uint16_t)(1<<4)},
                                           {(uint16_t * const)&LATA, (uint16_t)(1<<4)},
                                           {(uint16_t * const)&LATA, (uint16_t)(1<<9)}
                                        };

static const led_column_t led_columns_array[] = {
                                           {(uint16_t * const)&LATC, (uint16_t)(1<<3)},
                                           {(uint16_t * const)&LATC, (uint16_t)(1<<4)},
                                           {(uint16_t * const)&LATC, (uint16_t)(1<<5)}
                                        };
...
static volatile _Bool leds_matrix[3][3];
...
void LedsMatrixTask(void)
{
    ...
    {
        *(led_columns_array[column_counter].port) &= ~(led_columns_array[column_counter].mask);
    }


    *(led_rows_array[row_counter].port) |= (led_rows_array[row_counter].mask);
