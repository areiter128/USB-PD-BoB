...
static volatile button_data_t buttonArray[2];

void ButtonsInit(void)
{
    INTERRUPT_GlobalDisable();
    buttonArray[0].debouncer = 0;
    buttonArray[0].timer = 0;
    buttonArray[0].event = 0;
    buttonArray[0].state = 1;

    buttonArray[1].debouncer = 0;
    buttonArray[1].timer = 0;
    buttonArray[1].event = 0;
    buttonArray[1].state = 1;
    INTERRUPT_GlobalEnable();
}

static _Bool _ButtonRead(uint8_t id)
{
    switch(id)
    {
        case 0: return PORTAbits.RA2;
        case 1: return PORTAbits.RA3;
        default: return 1;
    }
}
...
void ButtonsTask(void)
{
  ...
  _Bool input_state = _ButtonRead(scan);
  _Bool last_state = buttonArray[scan].state;

  if(input_state == last_state)
  {
      buttonArray[scan].debouncer = 0;
      if((input_state == 0) && (buttonArray[scan].timer < (1500 / ( 5 * 2))))
      {
          buttonArray[scan].timer++;
          if(buttonArray[scan].timer == (1500 / ( 5 * 2)))
              buttonArray[scan].event = 2;
      }
  }
  else
  {
      buttonArray[scan].debouncer++;
      if(buttonArray[scan].debouncer == (30 / ( 5 * 2)))
      {
          buttonArray[scan].state = input_state;
          buttonArray[scan].debouncer = 0;
          if((input_state == 1) && (buttonArray[scan].timer < (1500 / ( 5 * 2))))
          {
              buttonArray[scan].event = 1;
          }
          buttonArray[scan].timer = 0;
      }
  }
  scan++; if(scan >= 2) scan = 0;
}
