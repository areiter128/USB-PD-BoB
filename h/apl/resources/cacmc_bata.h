...
#ifndef __SPECIAL_FUNCTION_LAYER_CACMC_BATA_H__
#define __SPECIAL_FUNCTION_LAYER_CACMC_BATA_H__
 ...
 * ***************************************************************************************/
	...
	} __attribute__((packed))CACMC_BATA_CONTROL_LOOP_COEFFICIENTS_t;

	typedef struct
	{
		volatile fractional ControlHistory[2];  // Control History
		volatile fractional ErrorHistory[3];    // Error History
	} __attribute__((packed))CACMC_BATA_CONTROL_LOOP_HISTORIES_t;


	extern volatile cNPNZ16b_t cacmc_bata; // user-controller data object

/* ***************************************************************************************/

// Function call prototypes for initialization routines and control loops

extern inline uint16_t cacmc_bata_Init(void); // Loads default coefficients into 2P2Z controller and resets histories to zero

extern inline void cacmc_bata_Reset( // Resets the 2P2Z controller histories
	volatile cNPNZ16b_t* controller // Pointer to nPnZ data structure
	);

extern inline void cacmc_bata_Precharge( // Pre-charges histories of the 2P2Z with defined steady-state data
	volatile cNPNZ16b_t* controller, // Pointer to nPnZ data structure
...
extern inline void cacmc_bata_Update( // Calls the 2P2Z controller
	volatile cNPNZ16b_t* controller // Pointer to nPnZ data structure
	);

#endif	// end of __SPECIAL_FUNCTION_LAYER_CACMC_BATA_H__ header file section
